...
# Families whose builders accept the stabiliser-pattern knobs (pattern/start/edge).
_VARIANT_FAMILIES = {"rotated_surface", "unrotated_surface"}

# Non-Pauli (coherent / amplitude-damping) channels carry signed branch weights no
# DetectorErrorModel can hold. The graphlike DEM decoders below genuinely can't
# decode them; the TN family ("tn"/"mld") and "coherent" handle them by contracting
# the circuit's signed quasiprobability branches instead (see make_circuit_decoder).
_NON_PAULI_CHANNELS = {"RZ", "RX", "AMPLITUDE_DAMP"}
_PAULI_ONLY_DECODERS = {"mwpm", "bposd"}
...
def _template_circuit_fn(
    ...
    """
    codes.<family>(distance, rounds, p, channel=...) curried over the physical
    rate p, with the requested noise channel threaded through. Surface families
    also take the stabiliser-pattern knobs (pattern/start/edge); the others ignore
    them.
    """
    ...
    return lambda p: builder(distance, rounds, p, channel=channel)
